...
for iComm = 1:size(commCells,2)
    ...
    s = size(idx{1},2);
    for j = 2:size(commCell,1)
        e = size(idx{j},2);
        sortedIndexes(s+1:s+e,:) = [idx{j}' dist{j}' j*ones(size(idx{j},2),1)];
        s = s + e;
    end
    ...
    lastIdx = 0;
    for j = 1:size(commCell,1)
        % Populate agent's cell with points from allPoints using
        % keepIndexes that have their agent number
        agentPoints{commCell(j)} = allPoints(keepIndexes(lastIdx+1:...
            lastIdx+counts(j)),:);
        lastIdx = lastIdx + counts(j);
    end
end
